...
def gf_mult_by_2(t: UInt8) -> UInt8:
    var lq = UInt32(t) << 1
    lq ^= ((lq >> 8) & 1) * 0x011B
    # KCipher-2 uses the complemented AES-field multiply value in its S-box
    # diffusion step; this is intentional and covered by the checked vectors.
...
def sub_k2(in_val: UInt32) -> UInt32:
    ...
    return (
        (UInt32(q3) << 24) | (UInt32(q2) << 16) | (UInt32(q1) << 8) | UInt32(q0)
    )
...
struct KCipher2:
    ...
    def _next_init(mut self):
        ...
        var temp1_amul1 = b0_shift ^ _amul1(Int(self.b0 >> 24))
        var temp1_amul2 = b0_shift ^ _amul2(Int(self.b0 >> 24))
        temp1 = self._select_u32(temp1_amul2, temp1_amul1, (old_a2 >> 30) & 1)

        var temp2_amul3 = ((self.b8 << 8) & 0xFFFFFF00) ^ _amul3(
            Int(self.b8 >> 24)
        )
