...
def _getrandom_linux_x86(buf: Pointer[mut=True, UInt8, _, address_space=_], length: Int) -> Int:
    # Linux x86-64 getrandom: syscall 318, flags = 0.
    # rax supplies the syscall number and receives the byte count or negative errno.
    return Int(
        inlined_assembly[
            "syscall",
            Int64,
...
def random_fill(buf: Span[mut=True, UInt8, ...]) raises:
    """Fill a span from OS entropy, retrying interruptions and propagating other failures."""
    var length = len(buf)
    if length == 0:
        return
...
def random_bytes(n: Int) raises -> List[UInt8]:
    """Return n bytes from OS entropy; n must be nonnegative."""
    if n < 0:
