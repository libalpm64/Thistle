...
from thistle.sha3 import sha3_256, sha3_512, shake128, shake256
...
def cbd3(mut r: Poly, buf: Span[UInt8, ...]) raises:
    ...
    for i in range(N // 4):
        var t = _u24_le(buf, 3 * i)
        var d = t & 0x00249249
        d += (t >> 1) & 0x00249249
        d += (t >> 2) & 0x00249249
        for j in range(4):
            var a = Int16((d >> UInt32(6 * j + 0)) & 0x7)
            var b = Int16((d >> UInt32(6 * j + 3)) & 0x7)
            r.coeffs[4 * i + j] = a - b
...
def cbd2(mut r: Poly, buf: Span[UInt8, ...]) raises:
    ...
    for i in range(N // 8):
        var t = _u32_le(buf, 4 * i)
        var d = t & 0x55555555
        d += (t >> 1) & 0x55555555
        for j in range(8):
            var a = Int16((d >> UInt32(4 * j + 0)) & 0x3)
            var b = Int16((d >> UInt32(4 * j + 2)) & 0x3)
            r.coeffs[8 * i + j] = a - b
...
def poly_basemul_montgomery(mut r: Poly, a: Poly, b: Poly):
...
def poly_add(mut r: Poly, a: Poly, b: Poly):
    comptime W = simd_width_of[DType.int16]()
...
def poly_add_inplace(mut r: Poly, b: Poly):
    comptime W = simd_width_of[DType.int16]()
    var rp = r.coeffs.unsafe_ptr()
    var bp = b.coeffs.unsafe_ptr()
...
def poly_sub(mut r: Poly, a: Poly, b: Poly):
    comptime W = simd_width_of[DType.int16]()
    var rp = r.coeffs.unsafe_ptr()
    var ap = a.coeffs.unsafe_ptr()
    var bp = b.coeffs.unsafe_ptr()
...
def poly_sub_from(mut r: Poly, a: Poly):
    comptime W = simd_width_of[DType.int16]()
    var rp = r.coeffs.unsafe_ptr()
    var ap = a.coeffs.unsafe_ptr()
...
def poly_tobytes_stack(mut out: StackBuffer[UInt8, INDCPA_PUBLICKEYBYTES_MAX], a: Poly):
    for i in range(N // 2):
        var t0 = _positive_coeff(a.coeffs[2 * i])
        var t1 = _positive_coeff(a.coeffs[2 * i + 1])
...
def polyvec_tobytes(mut out: List[UInt8], a: Polyvec, k: Int):
    for i in range(k):
        poly_tobytes(out, a.vec[i])


def polyvec_tobytes_stack(mut out: StackBuffer[UInt8, INDCPA_PUBLICKEYBYTES_MAX], a: Polyvec, k: Int):
    for i in range(k):
...
def polyvec_compress(mut out: List[UInt8], a: Polyvec, k: Int) raises:
...
def poly_compress(mut out: List[UInt8], a: Poly, k: Int) raises:
    if k == K_512 or k == K_768:
        for i in range(N // 8):
...
def polyvec_add(mut r: Polyvec, a: Polyvec, b: Polyvec, k: Int):
    for i in range(k):
        poly_add(r.vec[i], a.vec[i], b.vec[i])


def polyvec_basemul_acc_montgomery(mut r: Poly, a: Polyvec, b: Polyvec, k: Int):
    var t = Poly()
...
# Note this is O(n^2) need to add a streaming shake API at some point but not explicity required.
def sample_ntt(seed: Span[UInt8, ...], x: UInt8, y: UInt8) raises -> Poly:
    ...
    var ctr = 0
    while ctr < N:
        var stream = xof(seed, x, y, out_len)
        ctr += rej_uniform(out, ctr, Span[UInt8, ...](stream)[consumed:len(stream)])
        consumed = len(stream)
        out_len += 168
    return out^
...
def poly_getnoise_eta1_512(mut r: Poly, seed: Span[UInt8, ...], iv: UInt8) raises:
    var buf = prf(seed, iv, ETA1_512 * N // 4)
    poly_cbd_eta1_512(r, Span[UInt8, ...](buf))


def poly_getnoise_eta1(mut r: Poly, seed: Span[UInt8, ...], iv: UInt8) raises:
    var buf = prf(seed, iv, ETA1 * N // 4)
    poly_cbd_eta1(r, Span[UInt8, ...](buf))


def poly_getnoise_eta2(mut r: Poly, seed: Span[UInt8, ...], iv: UInt8) raises:
    var buf = prf(seed, iv, ETA2 * N // 4)
    poly_cbd_eta2(r, Span[UInt8, ...](buf))
...
def gen_matrix(mut a: InlineArray[Polyvec, K_MAX], seed: Span[UInt8, ...], transposed: Bool, k: Int) raises:
    for i in range(k):
        for j in range(k):
            if transposed:
                a[i].vec[j] = sample_ntt(seed, UInt8(i), UInt8(j))
            else:
                a[i].vec[j] = sample_ntt(seed, UInt8(j), UInt8(i))
...
def pack_pk(mut out: List[UInt8], pk: Polyvec, seed: InlineArray[UInt8, SYMBYTES], k: Int) -> Bool:
...
def pack_pk_stack(mut out: StackBuffer[UInt8, INDCPA_PUBLICKEYBYTES_MAX], pk: Polyvec, seed: InlineArray[UInt8, SYMBYTES], k: Int) -> Bool:
    if polyvec_byte_size(k) == 0:
        return False
...
def pack_sk(mut out: List[UInt8], sk: Polyvec, k: Int) -> Bool:
    if polyvec_byte_size(k) == 0:
        return False
...
def pack_ciphertext(mut out: List[UInt8], b: Polyvec, v: Poly, k: Int) raises -> Bool:
...
def k_pke_keygen(mut ek: KPKEEncryptionKey, mut dk: KPKEDecapsulationKey, d: Span[UInt8, ...], k: Int) raises:
    ...
    var g = hash_g(Span[UInt8, ...](ptr=g_input.ptr(), length=g_input.len()))
    var rho = g[0:SYMBYTES]
    var sigma = g[SYMBYTES:2 * SYMBYTES]
...
def k_pke_encrypt(mut ciphertext: List[UInt8], ek: KPKEEncryptionKey, m: Span[UInt8, ...], r: Span[UInt8, ...]) raises -> Bool:
...
def k_pke_decrypt_msg(mut plaintext: StackBuffer[UInt8, SYMBYTES], dk: KPKEDecapsulationKey, c: Span[UInt8, ...]) raises -> Bool:
    var k = dk.k
    if k != K_512 and k != K_768 and k != K_1024:
        return False
...
def kem_keygen_internal(seed: Span[UInt8, ...], k: Int) raises -> DecapsulationKey:
    ...
    var h = hash_h(Span[UInt8, ...](ptr=ek_bytes.ptr(), length=ek_bytes.len()))
    for i in range(SYMBYTES):
        dk.ek.h[i] = h[i]
        dk.z[i] = z[i]
    return dk^
...
def decapsulation_key_expanded_bytes(dk: DecapsulationKey) -> List[UInt8]:
...
def encapsulation_key_bytes(dk: DecapsulationKey) -> List[UInt8]:
...
def decapsulation_key_decode(mut dk: DecapsulationKey, input: Span[UInt8, ...], k: Int) raises -> Bool:
    ...
    var h = hash_h(input[sk_len:sk_len + ek_len])
    var h_diff = UInt8(0)
    for i in range(SYMBYTES):
...
def mlkem_keygen_seed(seed: Span[UInt8, ...], parameter_set: String) raises -> Tuple[List[UInt8], List[UInt8]]:
    ...
    return (encapsulation_key_bytes(dk), decapsulation_key_expanded_bytes(dk))
...
def mlkem_encaps_seed_vector_order(ek_bytes: Span[UInt8, ...], m: Span[UInt8, ...], parameter_set: String) raises -> Tuple[List[UInt8], List[UInt8], Bool]:
    ...
    for i in range(SYMBYTES):
        shared.append(g[i])
    var ciphertext = List[UInt8](capacity=polyvec_compressed_byte_size(k) + poly_compressed_bytes(k))
    if not k_pke_encrypt(ciphertext, ek, m, Span[UInt8, ...](g[SYMBYTES:2 * SYMBYTES])):
        _zero_list(g)
        _zero_list(shared)
        return (List[UInt8](), List[UInt8](), False)
    _zero_list(g)
...
def mlkem_decaps(dk_bytes: Span[UInt8, ...], ciphertext: Span[UInt8, ...], parameter_set: String) raises -> Tuple[List[UInt8], Bool]:
    ...
    var ct_check = List[UInt8](capacity=_ciphertext_size(k))
    if not k_pke_encrypt(ct_check, dk.ek.pke_ek, Span[UInt8, ...](ptr=m.ptr(), length=m.len()), Span[UInt8, ...](g[SYMBYTES:2 * SYMBYTES])):
        _zero_stack_u8(m)
        _zero_list(g)
        _zero_list(ct_check)
        return (List[UInt8](), False)

    var rejection = rkprf(Span[UInt8, ...](dk.z), ciphertext)
    var shared_buf = StackBuffer[UInt8, SYMBYTES]()
    var equal = _ct_is_zero_u8(_bytes_diff(Span[UInt8, ...](ct_check), ciphertext))
    for i in range(SYMBYTES):
        shared_buf.push_unchecked(_ct_select_u8(rejection[i], g[i], equal))
