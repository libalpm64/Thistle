...
def hchacha20(
    ...
    out_ptr.bitcast[UInt32]().store[alignment=1](0, row0)
    (out_ptr + 16).bitcast[UInt32]().store[alignment=1](0, row3)
...
def _aead_core[encrypt: Bool](
    ...
    input: Span[UInt8, ...],
    output: UnsafePointer[mut=True, UInt8, _, address_space=_],
    tag: UnsafePointer[mut=True, UInt8, _, address_space=_],
) raises:
    var key_bytes = key.unsafe_ptr().load[width=32, alignment=1](0)
    var nonce_bytes = SIMD[DType.uint8, 16](0)
    for i in range(12):
        nonce_bytes[i] = nonce[i]

    var kw = bitcast[DType.uint32, 8](key_bytes)
    var nw = bitcast[DType.uint32, 4](nonce_bytes)
...
def xchacha20_poly1305_encrypt(
    ...
    chacha20_poly1305_encrypt(
        ...
        Span[UInt8, ...](unsafe_ptr=sp + 32, length=12),
